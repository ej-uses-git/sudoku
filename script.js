class Cell {
  constructor(stringId) {
    ...
    this._boxNumber =
      (Math.floor(this._id / 3) % 3) + Math.floor(this._rowNumber / 3) * 3;
  ...
  get asElement() {
    return this._asElement;
  }
  get solverValue() {
    return this._solverValue;
  }
  get deletable() {
    return this._deletable;
  }
  //setters
  set storedValue(input) {
    this._storedValue = input;
  }
  set displayValue(input) {
    this._displayValue = input;
  }
  set solverValue(input) {
    this._solverValue = input;
  }
  set deletable(input) {
    this._deletable = input;
  }
...
//general functions
const findObjectFromElement = (elem, arr) =>
  arr[elem.parentElement.id.slice(elem.parentElement.id.indexOf("_") + 1)];
...
const isInRow = (rows, cell, num) =>
  seeSolverVal(rows[cell.rowNumber]).includes(num);
const isInCol = (columns, cell, num) =>
  seeSolverVal(columns[cell.colNumber]).includes(num);
const isInBox = (boxes, cell, num) =>
  seeSolverVal(boxes[cell.boxNumber]).includes(num);
const legal = (rows, columns, boxes, cell, num) =>
  !isInRow(rows, cell, num) &&
  !isInCol(columns, cell, num) &&
  !isInBox(boxes, cell, num);
...
const markerSetter = (markers, rows, checker) => {
  ...
  for (let i = 1; i <= 9; i++) {
    ...
    if (
      rowsInputs.filter((row) => row.includes(i)).length === 9 &&
      !markers[i - 1].className.includes("hidden")
    ) {
  ...
  for (let row of rows) {
    for (let cell of row) {
      cell.asElement.setAttribute(
        "oninput",
        `this.value=this.value.replace(/${markedNumRegex}/g,\'\');`
      );
    }
  }
};
...
const createPuzzle = (cells, numOfFilledCells) => {
  if (numOfFilledCells < 17)
    throw new Error(
      "a sudoku board with less than 17 filled cells cannot have only one solution"
    );
  ...
  cells
    .filter((cell) => cell.displayValue !== 0)
    .forEach((cell) => {
...
if (
  localStorage.hasOwnProperty("currentPuzzle") &&
  localStorage.hasOwnProperty("currentSolution")
) {
  ...
  diffScreen.className = "hidden";
  winScreen(
    localStorage.getItem("mistakeCounter"),
    localStorage.getItem("puzzlesolved")
  );
...
cells.forEach((cell) =>
  cell.asElement.addEventListener("focus", () => cell.clearInput())
);
  ...
  if (!winScreenActive) {
    ...
    if (e.code.includes("Enter")) {
      ...
      cells
        .filter(
          (cell) =>
            !cell.asElement.hasAttribute("disabled") &&
            cell.asElement.value !== ""
        )
        .forEach((cell) => {
      ...
      if (newCellEToFocus) newCellEToFocus.focus();

      markerSetter(markers, rows);

      if (
        cells.filter((cell) => cell.asElement.hasAttribute("disabled"))
          .length === 81
      ) {
        let newpuzzButton = document.getElementById("new_puzzle");
        ...
        let miliseconds;
        let milisecondsCounted =
          Number(localStorage.getItem("milisecondsCounted")) || 0;
        if (milisecondsCounted)
          miliseconds = milisecondsCounted + (endTime - loadTime);
        else miliseconds = endTime - Number(localStorage.getItem("startTime"));
        ...
        winScreen(
          localStorage.getItem("mistakeCounter"),
          `${minutesFirstDigit}${minutesSecondDigit}:${secondsFirstDigit}${secondsSecondDigit}`
        );
        winScreenActive = true;
      ...
      if (proceed) {
        ...
        let currentColumn = emptyCells.filter(
          (cell) => cell.colNumber === currentObject.colNumber
        );
        let currentRow = emptyCells.filter(
          (cell) => cell.rowNumber === currentObject.rowNumber
        );
        let found = false;
        ...
        if (e.code.includes("Up")) {
          if (currentColumn[currentColumn.indexOf(currentObject) - 1]) {
            currentColumn[
              currentColumn.indexOf(currentObject) - 1
            ].asElement.focus();
          } else {
            for (let i = currentObject.colNumber + 1; i < 9; i++) {
              let nextColumn = emptyCells
                .filter(
                  (cell) =>
                    cell.colNumber === i &&
                    cell.rowNumber < currentObject.rowNumber
                )
                .reverse();
              if (nextColumn[0]) {
          ...
          if (currentRow[currentRow.indexOf(currentObject) + 1]) {
            ...
            for (let i = currentObject.rowNumber + 1; i < 9; i++) {
              let nextRow = emptyCells.filter(
                (cell) =>
                  cell.rowNumber === i &&
                  cell.colNumber > currentObject.colNumber
              );
              if (nextRow[0]) {
          ...
          if (currentColumn[currentColumn.indexOf(currentObject) + 1]) {
            ...
            for (let i = currentObject.colNumber - 1; i >= 0; i--) {
              let lastColumn = emptyCells.filter(
                (cell) =>
                  cell.colNumber === i &&
                  cell.rowNumber > currentObject.rowNumber
              );
            ...
            if (!found) {
              for (let i = currentObject.colNumber + 1; i < 9; i++) {
                let nextColumn = emptyCells.filter(
                  (cell) =>
                    cell.colNumber === i &&
                    cell.rowNumber > currentObject.rowNumber
                );
...
shareButton.addEventListener("click", () => {
  if (winScreenActive) {
    statsDisplay.textContent = `Mistakes: ${localStorage.getItem(
      "mistakeCounter"
    )} // Time Elapsed: ${localStorage.getItem("puzzlesolved")}`;
    ...
    copyStats.addEventListener("click", () => {
      if (copyclicked === false) {
        ...
        navigator.clipboard.writeText(
          `I solved puzzle #${websitePuzzleID(
            officialSudokuString(cells)
          )} at difficulty level ${localStorage.getItem(
            "textpermadiff"
          )} over at EJ's Sudoku Site!\n\nMistakes: ${localStorage.getItem(
            "mistakeCounter"
          )} // Time Elapsed: ${localStorage.getItem(
            "puzzlesolved"
    ...
    copyString.addEventListener("click", () => {
      if (copyclicked === false) {
        copyclicked = true;
        navigator.clipboard.writeText(
          `${localStorage.getItem("officialString")}`
        );
        setTimeout(() => {
          alert(
            "Copied the empty puzzle to the clipboard (as an official sudoku string)!"
          );
          copyclicked = false;
        }, 400);
      }
